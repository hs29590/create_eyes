...
class LineTracker
{
    ...
    bool debug;			// Debugging on or off.
    bool show_images;	// Show images or not.
    ...
    vector<Point> line_points;	// vector om de laatste gevonden punten bij te houden.
    vector<Point> old_line_points;
    Point first_scanpoint;		// Eerst gevonden punt.

    int scan_counter;		// Variable to count how many scans are done.

    short scan_radius;
    short sc_strt, sc_end;

    short new_scan_radius1;

    // Variables for trackbars.
    static const int scan_height_reg = 220;
    static const int scan_posx_reg = 160;
    static const int scan_radius1_reg = 55;
    static const int scan_radius2_reg = 18;
    static const int look_angle_reg = 180;
    static const int look_width_reg = 160;

    static const int H_min_reg = 50;
    static const int H_max_reg = 87;
    static const int S_min_reg = 71;
    static const int S_max_reg = 256;	// 133
    static const int V_min_reg = 80;		// 90
    static const int V_max_reg = 256;	// 197
    ...
    public:
    LineTracker()
        : it_(nh_),line_points(7),old_line_points(7)
    {
        // Subscrive to input video feed and publish output video feed
        image_sub_ = it_.subscribe("/camera/rgb/image_raw", 1,
                &LineTracker::imageCb, this);
        //image_pub_ = it_.advertise("/image_converter/output_video", 1);

        //cv::namedWindow(OPENCV_WINDOW);
        scan_image = Mat( 256, 640, CV_8UC3, Scalar( 255, 255, 255));		// to graph greyscale values on.
        D_scan_image = Mat( 256, 640, CV_8UC3, Scalar( 255, 255, 255));		// to graph the differences in greyscale values.

        erodeElement = getStructuringElement( MORPH_RECT, Size(4,4));	// matrix element to erode image - eroding removes small spots.

        green_x = 160;
        green_y = 120;

        debug = 1;			// Debugging on or off.
        show_images = 1;	// Show images or not.
    ...
    ~LineTracker()
    {
        //cv::destroyWindow("camera image"i);
        destroyAllWindows();
    }
    ...
    void imageCb(const sensor_msgs::ImageConstPtr& msg)
    {
        ...
        image = cv_ptr->image;
        //image = imread("/home/harshit/pyTrials/VisionRace/2_scale.jpeg");
        ...
        if(show_images==1){
            ...
            imshow( "thresh image", eroded_image );
        }
        ...
        //for(int cc = 0; cc < line_points.size(); cc++)
        // {
        //    cout << line_points[cc] << " ";
        // }
        //cout << endl;
...
bool LineTracker::checkJump()
{
    ...
    {
        sum = sum + abs(old_line_points[i].x - line_points[i].x) + abs(old_line_points[i].y - line_points[i].y);
    }

    cout << "sum: " << sum << endl;

    old_line_points = line_points;
...
void LineTracker::scancircle( Mat& I, Point Mp, int radius, int look_angle, int width ){
    int i,j;
    int n = 0;
...
void LineTracker::scanline( Mat& I, Point Mp, int line_radius ){
...
Point LineTracker::findLine( char scan_mode )
{
    Point left_side;
    Point right_side;
...
void LineTracker::drawObject(int x, int y,Mat &frame, Scalar color){
    /* by Kyle Hounslow */
...
void LineTracker::trackObject(int *x, int *y, Mat &workImage){
    /* by Kyle Hounslow */
...
bool LineTracker::test_inimage( Mat& I, short x_cor, short y_cor ){
    if( x_cor >= 0 && y_cor >=0 && x_cor < I.cols && y_cor < I.rows ) return 1;
    else return 0;
}

int LineTracker::lineangle( void ){
